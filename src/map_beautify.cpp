...
void remove_small_components(const cv::Mat & input, cv::Mat & output, int area_thresh){
    cv::Mat labels, status, centroids;
    int nccomps = cv::connectedComponentsWithStats(input, labels, status, centroids);
...
int main(int argc, char **argv) {
  ...
  cv::Mat map_ob = cv::Mat::zeros(map_image.rows, map_image.cols, map_image.type());
  cv::Mat map_un = cv::Mat::zeros(map_image.rows, map_image.cols, map_image.type());
  cv::Mat map_fr = cv::Mat::zeros(map_image.rows, map_image.cols, map_image.type());
  ...
  cv::Mat map_fr_open;
  cv::morphologyEx(map_fr, map_fr_open, cv::MORPH_OPEN,
                   open_element_w);
  cv::imshow("map_fr_open", map_fr_open);

  cv::Mat map_fr_filtered;
  remove_small_components(map_fr_open, map_fr_filtered, FREESPACE_FILTER_THRESH);
  cv::imshow("map_fr_filtered", map_fr_filtered);

  /* Close freespace */
  int close_kernel_size_w = 1;
  ...
  cv::findContours(map_fr_close, contours, hierarchy, cv::RETR_TREE,
                   cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0));

  for (size_t i = 0; i < contours.size(); i++)
    cv::drawContours(map_combine_with_contours, contours, (int)i, cv::Scalar(0, 0, 0), 1);
